...
def compute_mean_ioU(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val'):
    list_path = os.path.join(datadir, dataset, 'id.txt')
    val_id = [i_id.strip() for i_id in open(list_path)]

    confusion_matrix = np.zeros((num_classes, num_classes))

    for i, im_name in enumerate(val_id):
        gt_path = os.path.join(datadir, dataset, 'gt', im_name + '.png')
...
if __name__ == "__main__":
    ...
    # print(im.shape)
    # val = np.asarray( PILImage.open(im_path))
    # print(val.shape)
    # if im.all()!=val.all():
    #     print('different')
    # output_im = PILImage.fromarray(np.zeros((100,100), dtype=np.uint8))
    # output_im.putpalette(palette)
    # output_im.save('val.png')
    pred_dir = 'snapshots/results/epoch4/'
    num_classes = 20
    datadir = 'data/LIP'
