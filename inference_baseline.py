...
parser = argparse.ArgumentParser(description="CE2P Baseline Inference")
parser.add_argument("--gpu", type=str, default='0', help="choose gpu device.")
parser.add_argument("--restore-from", type=str, default='./snapshots_384/base_bs8/LIP_epoch_149.pth',
                    help="Where restore model parameters from.")
parser.add_argument("--save-root", type=str, default='result_baseline')
...
def save_img(preds, coarse_preds, edge_preds, scales, centers, input_size):
    ...
    colormap_edge = [(0, 0, 0), (255, 255, 255)]
    ...
    for i, im_name in enumerate(val_id):
        ...
        save_path = os.path.join(args.save_root, '{}'.format(i))
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        cv2.imwrite(os.path.join(save_path, '{}.png'.format(im_name)), ori_img)

        pred_out = preds[i]
        pred = transform_parsing(pred_out, c, s, w, h, input_size)
        parsing_pred = np.asarray(pred, dtype=np.int32)
        color_parsing_pred = cam_mask(parsing_pred, colormap, args.num_classes)
        # color_parsing_pred.save('result/{}/{}_parsing_pred.png'.format(i, im_name))
        cv2.imwrite(os.path.join(save_path, '{}_parsing_pred.png'.format(im_name)), color_parsing_pred)
        ...
        color_coarse_parsing_pred = cam_mask(coarse_parsing_pred, colormap, args.num_classes)
        # color_coarse_parsing_pred.save('result/{}/{}_coarse_parsing_pred.png'.format(i, im_name))
        cv2.imwrite(os.path.join(save_path, '{}_coarse_parsing_pred.png'.format(im_name)), color_coarse_parsing_pred)
        ...
        color_edge_pred = cam_mask(edge_pred, colormap_edge, 2)
        # color_edge_pred.save(os.path.join(save_path, '{}_edge_pred.png'.format(i, im_name)))
        cv2.imwrite(os.path.join(save_path, '{}_edge_pred.png'.format(im_name)), color_edge_pred)

        parsing_gt = np.asarray(parsing_gt, dtype=np.int32)
        color_parsing_gt = cam_mask(parsing_gt, colormap, args.num_classes)
        # color_parsing_gt.save('result/{}/{}_parsing_gt.png'.format(i, im_name))
        cv2.imwrite(os.path.join(save_path, '{}_parsing_gt.png'.format(im_name)), color_parsing_gt)

        edge_gt = np.asarray(edge_gt, dtype=np.int32)
        edge_gt = cam_mask(edge_gt, colormap_edge, 2)
        # edge_gt.save('result/{}/{}_edge_gt.png'.format(i, im_name))
        cv2.imwrite(os.path.join(save_path, '{}_edge_gt.png'.format(im_name)), edge_gt)
    ...
    with torch.no_grad():
        for index, batch in enumerate(valloader):
            ...
            if gpus > 1:
                ...
                parsing = outputs[0][-1]
                # print(parsing.shape)
                parsing = interp(parsing.unsqueeze(0)).data.cpu().numpy()
                parsing = parsing.transpose(0, 2, 3, 1)  # NCHW NHWC
                parsing_preds[idx:idx + num_images, :, :] = np.asarray(np.argmax(parsing, axis=3), dtype=np.uint8)

                coarse_parsing = outputs[0][0]
                coarse_parsing = interp(coarse_parsing.unsqueeze(0)).data.cpu().numpy()
                coarse_parsing = coarse_parsing.transpose(0, 2, 3, 1)  # NCHW NHWC
                coarse_parsing_preds[idx:idx + num_images, :, :] = np.asarray(np.argmax(coarse_parsing, axis=3),
                                                                              dtype=np.uint8)

                edge = outputs[1][0]
                edge = interp(edge.unsqueeze(0)).data.cpu().numpy()
...
def main():
    ...
    parsing_preds, coarse_parsing_preds, edge_preds, scales, centers = valid(model, valloader, input_size, num_samples,
                                                                             len(gpus))
